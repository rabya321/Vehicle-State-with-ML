...
class GRURegressor(nn.Module):
    ...
    def forward(self, x):
        # GRU expects shape: (batch_size, seq_len, input_size)
...
model = GRURegressor(input_size, hidden_size, num_layers, output_size, dropout)
...
for epoch in range(epochs):
    ...
    if avg_val_loss < best_val_loss:
        ...
        print(f"Best GRU model saved at epoch {epoch+1}.")
    else:
